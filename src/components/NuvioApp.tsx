...
import { Bell, BookOpen, Brain, BriefcaseBusiness, Check, ChevronRight, Edit2, FileText, FolderKanban, Grid2X2, LayoutDashboard, Menu, MessageSquare, Mic, Paperclip, Pencil, Plus, Search, Send, Settings, Sparkles, Trash2, Users, X, Zap } from "lucide-react";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";

import { Sidebar, Page, navItems } from "@/components/Sidebar";
...
function Home({ messages, input, setInput, send, loading, voice, voiceStatus, level, onVoice, onStopVoice, onPreview, pendingFiles, setPendingFiles, chatFileRef, onChatFile, feedEnd }: { messages: Message[]; input: string; setInput: (v:string)=>void; send:(v?:string)=>void; loading:boolean; voice:boolean; voiceStatus:"idle"|"listening"|"error"; level:number; onVoice:()=>void; onStopVoice:()=>void; onPreview:(n:string,id?:string)=>void; pendingFiles: File[]; setPendingFiles: (f: File[])=>void; chatFileRef: React.RefObject<HTMLInputElement | null>; onChatFile: (e: React.ChangeEvent<HTMLInputElement>)=>void; feedEnd: React.RefObject<HTMLDivElement | null> }) {
  ...
  return <div className={`assistant-stage ${landing ? "landing" : "chatting"}`}><AIAmbientBackground active={!landing || voice} isGenerating={loading} isVoiceActive={voiceStatus === "listening"} /><AnimatePresence mode="wait">{voice ? <motion.div key="voice" className="voice-mode" initial={{opacity:0, scale:.97}} animate={{opacity:1, scale:1}} exit={{opacity:0, scale:.97}}><span className="voice-label">{voiceStatus === "error" ? "Microphone unavailable" : "Listening…"}</span><AIOrb state={orbState} level={level}/><p>{voiceStatus === "error" ? "Allow microphone access, then try again." : "Speak naturally. I'm listening."}</p><button className="voice-stop" onClick={onStopVoice}><Mic size={21}/><span>End voice session</span></button></motion.div> : landing ? <motion.div key="landing" className="hero" initial={{opacity:0}} animate={{opacity:1}}><div className="hero-grid"/><AIOrb state={orbState}/><div className="hero-copy"><h2>Hey, what can I help you <em>find?</em></h2><p>Search your knowledge. Talk to your second brain.</p></div><div className="suggestions">{defaultPrompts.map(p => <button key={p} onClick={() => send(p)}><Sparkles size={14}/>{p}</button>)}</div></motion.div> : <motion.div key="chat" className="chat-feed" initial={{opacity:0, y:12}} animate={{opacity:1,y:0}}>{messages.map((m,i) => <div className={`message message--${m.role}`} key={i}>{m.role === "assistant" && <span className="message-orb"><AIOrb size="small" state="success"/></span>}<div className="message-card">{m.attachments && m.attachments.length > 0 && <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:6}}>{m.attachments.map((a,j) => <span key={j} style={{fontSize:11,background:"rgba(99,102,241,0.15)",border:"1px solid rgba(99,102,241,0.25)",borderRadius:12,padding:"2px 8px",display:"inline-flex",alignItems:"center",gap:4}}><FileText size={12}/>{a.name}</span>)}</div>}{m.role === "user" ? <p>{m.text}</p> : <div className="markdown-content"><ReactMarkdown remarkPlugins={[remarkGfm]}>{m.text}</ReactMarkdown></div>}{m.sources && <div className="sources"><span>Grounded in your knowledge</span>{m.sources.map(s => <button key={s.title + s.docId} className="source" onClick={() => onPreview(s.title, s.docId)}><FileText size={18}/><div><b>{s.title}</b><small>{s.type} · {s.detail}</small></div><ChevronRight size={16}/></button>)}</div>}</div></div>)}{loading && <div className="retrieval"><AIOrb size="small" state="thinking"/><div><b>Searching your knowledge</b><span>Retrieving relevant memories and documents…</span></div></div>}<div ref={feedEnd}/></motion.div>}</AnimatePresence>{!voice && <>{pendingFiles.length > 0 && <div style={{display:"flex",flexWrap:"wrap",gap:6,padding:"0 16px 6px"}}>{pendingFiles.map((f,i) => <span key={i} style={{fontSize:11,background:"rgba(99,102,241,0.15)",border:"1px solid rgba(99,102,241,0.25)",borderRadius:14,padding:"3px 10px",display:"inline-flex",alignItems:"center",gap:4,color:"rgba(255,255,255,0.8)"}}><FileText size={12}/>{f.name.length > 20 ? f.name.slice(0,18)+"..." : f.name}<button onClick={() => setPendingFiles(pendingFiles.filter((_,j)=>j!==i))} style={{background:"none",border:"none",color:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:14,lineHeight:1}}>×</button></span>)}</div>}<div className="composer"><button aria-label="Attach document" onClick={() => chatFileRef.current?.click()}><Paperclip size={19}/></button><input ref={chatFileRef} type="file" multiple accept=".pdf,.doc,.docx,.txt,.csv,.jpg,.jpeg,.png,.gif,.webp" onChange={onChatFile} style={{display:"none"}} /><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key === "Enter" && send()} placeholder={landing ? "Ask anything about your knowledge…" : "Continue the conversation…"}/><button className="mic" onClick={onVoice} aria-label="Start voice mode"><Mic size={18}/></button><button className="send" onClick={() => send()} aria-label="Send message"><Send size={17}/></button></div></>}<p className="privacy"><Zap size={12}/> Private by design · Your knowledge stays yours</p></div>;
}
